(()=>{function h(n){return n.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}var g=/([\w.+-]+@[\w-]+\.[a-z][\w]*(?:\.[a-z][\w]*)*)|(\bhttps?:\/\/[^\s<)]+|\b(?:[a-z0-9-]+\.)+(?:com|dev|app|io|ai|org|net)\b(?:\/[^\s<),]*)?)/gi;function f(n){return n.replace(g,(e,s)=>{let t=e.replace(/[.,]$/,""),r=e.slice(t.length);return s?`<a href="mailto:${t}">${t}</a>${r}`:`<a href="${t.startsWith("http")?t:`https://${t}`}" target="_blank" rel="noopener">${t}</a>${r}`})}function p(n){let e=h(n.trim()).split(`
`),s=[],t=!1;for(let r of e){let a=r.trim(),i=/^[-*•]\s+(.*)/.exec(a);if(i){t||(s.push("<ul>"),t=!0),s.push(`<li>${i[1]}</li>`);continue}t&&(s.push("</ul>"),t=!1),a&&s.push(`<p>${a}</p>`)}return t&&s.push("</ul>"),f(s.join("")).replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>")}var b="/api/chat",v=["What's Kalkidan's experience with AI and LLMs?","Has Kalkidan worked at large companies?","What has Kalkidan built and shipped?","How can I get in touch with Kalkidan?"],E=document.getElementById("app");E.innerHTML=`
  <header class="top">
...
`;var l=document.getElementById("scroll"),y=document.getElementById("thread"),w=document.getElementById("hero"),L=document.getElementById("chips"),m=document.getElementById("form"),o=document.getElementById("input"),d=document.getElementById("send");for(let n of v){let e=document.createElement("button");e.type="button",e.className="chip",e.textContent=n,e.addEventListener("click",()=>{o.value=n,m.requestSubmit()}),L.appendChild(e)}function u(n,e,s){let t=document.createElement("div");if(t.className=`row ${n}`,n==="bot"){let i=document.createElement("div");i.className="avatar",i.textContent="\u1272",t.appendChild(i)}let r=document.createElement("div");r.className="body";let a=document.createElement("div");if(a.className="text",a.textContent=e,r.appendChild(a),s!=null&&s.length){let i=document.createElement("div");i.className="sources",i.innerHTML=s.map(c=>`<span>${c}</span>`).join(""),r.appendChild(i)}return t.appendChild(r),y.appendChild(t),l.scrollTop=l.scrollHeight,a}async function k(n){var s,t;w.style.display="none",u("user",n),d.disabled=!0;let e=u("bot","");e.classList.add("thinking"),e.textContent="Thinking\u2026";try{let r=await fetch(b,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({question:n})}),a=await r.json();if(e.classList.remove("thinking"),r.ok&&a.answer){if(e.innerHTML=p(a.answer),(s=a.sources)!=null&&s.length){let i=document.createElement("div");i.className="sources",i.innerHTML=a.sources.map(c=>`<span>${c}</span>`).join(""),e.parentElement.appendChild(i)}}else e.textContent=(t=a.error)!=null?t:"Sorry, something went wrong. Please try again."}catch{e.classList.remove("thinking"),e.textContent="Couldn't reach the server \u2014 please try again in a moment."}finally{d.disabled=!1,l.scrollTop=l.scrollHeight,o.focus()}}m.addEventListener("submit",n=>{n.preventDefault();let e=o.value.trim();!e||d.disabled||(o.value="",o.style.height="auto",k(e))});o.addEventListener("keydown",n=>{n.key==="Enter"&&!n.shiftKey&&(n.preventDefault(),m.requestSubmit())});o.addEventListener("input",()=>{o.style.height="auto",o.style.height=`${Math.min(o.scrollHeight,160)}px`});o.focus();})();
